...
# SESSION 6 (2026-08-04 pm → 08-06) — carrier rig: three root causes, overnight PASS, 6.12/rc promoted

## Rig root cause 1: our charger driver removed the board's only supply

Symptom: 6/6 boots reset, PON reporting UVLO, the first one at 8.10 s exactly
as the WCNSS pronto powered up. Stripping the SoC down (radios off, display
off, GPU off, one core, a 652.8 MHz cap) did **not** fix it — which is the tell
that the load was never the problem.

Cause, in our own `qcom_smbb` code: with no battery detected it cleared
`CTRL_EN`. On PM8941 there is no separate power-path FET — USB_IN reaches VPH
*through* the charge path — so "don't charge" and "don't power the board" are
the same register bit. Mainline leaves the path armed at the DT limit and boots
this same fixture with all three remoteprocs and WLAN up.

Fix (`e7e3fd91ef5e`): keep `CTRL_EN` armed and **leave the configured input
limit alone** when no battery is present; keep it off only for a battery that
is present but out of range (`bat_ready`), which is the actual safety case;
clear latched gates *before* evaluating `!input` (a latched CHG_GONE made a
present cable read absent). This is also a real phone fix: a device with a dead
or missing pack now boots on a charger.

Retraction: `qcom,charging-disabled` on the rig DT (tried the same morning) was
the same mistake in DT form.

## Rig root cause 2: `qcom,minimum-input-voltage` folded back the only supply

The load-onset deaths were not a power *budget* — they were the input-droop
floor. With no cell, VPH *is* USBIN through the charge path, so as an onset
droops the input toward `VIN_MIN` the buck throttles the input to defend the
floor and takes VPH down through PMIC UVLO (~3.4 V) with nothing to cover the
deficit. A phone never notices; its pack does.

Measured with an onset reproducer (schedutil, idle board, four workers started
in the same instant), counting deaths over attempts:

| `VIN_MIN` | attempts | deaths |
|---|---|---|
| 4.45 V (FP2 value, from `da281bf9345a`, no stated rationale) | 4 | **3** (PON UVLO/SMPL) |
| 4.30 V | 17 → later 40 | **0** |

At 4.30 V the input droops *further* (4.63 V vs 4.66 V) and carries on — the
signature of not folding back. Six sibling mainline msm8974 boards already use
4.20–4.30 V. **Rig DTB only** (`e609c83e4380`); the phone keeps the vendor
value until it is validated with a pack.

Refuted along the way, do not re-chase: the vendor does not rate-limit
frequency ramps; the frequency edge is not the trigger (a simultaneous
four-core 300 → 1497.6 MHz step on busy cores survives at 4.683 V); "the
fixture cannot carry radios + DVFS" was this same fold-back.

## Rig root cause 3: our own XPU err-fatal arming turned a tolerated fault into a reset

With the supply fixed, the rig still died — PS_HOLD, twice in ~16 min of
*idle*, with ramoops showing all four cores parked at 300 MHz, the gang rail
static (`vdd_writes` frozen, `vdd_timeouts` 0, `volt_sel == volt_sel_req`),
65 °C, 4.9 V. The kernel doing nothing and TZ resetting the SoC anyway.

`qcom,xpu-err-fatal` (ours, matching the vendor's unconditional arming in
`scm-xpu.c`) makes TrustZone treat an XPU permission violation as fatal.
Disarming it on the rig DTB (`937c0a3a5cbb`) ran **43 min idle + 40 min
four-core load at 86–88 °C with no reset**.

The violation itself is not ours and is not fixed by this — mainline commits it
silently because mainline never arms the fatal behaviour. **Open:** identify the
violating master and region. TZ records the offender in its diag region at
`0xfe805720` (vendor `tz_log`), which mainline does not map; expose it via an
`mmio-sram` child with `export`, never `/dev/mem` (see
[[devmem-xpu-hazard]] — reading RPM MSG RAM/IMEM resets the SoC).

Note the original justification for arming it (that the unarmed SBL default
silently PS_HOLD-resets under load) no longer reproduces, most plausibly
because the real cause there was the SPM vsel cache poisoning fixed in
`c1076b55dd05`.

## Overnight soak — PASS (the §1.1 stability gate, and then some)

Single boot of **6 h 45 min**, self-sustained (host powered off), 803 fsync'd
samples in 30 min load / 10 min idle phases:

- **267,389** per-core frequency transitions, **10** distinct OPPs exercised
- **86,931** gang-rail writes, `vdd_timeouts 0`, `volt_sel == volt_sel_req`
  throughout
- 64 → 88 °C (thermal governor bounding load, not the supply)
- minimum supply 4.751 V, **zero resets**, no unexplained boots

Nothing caps the cores: `rig-envelope` was removed entirely (it twice
mis-fired and pinned the fixture at 300 MHz — once because `RIG_SAG_PCT` had
no default, so the trip point equalled the idle baseline), `maxcpus` and the
`modprobe.blacklist` staging are gone, and the rig boot entry now carries only
`cpufreq.default_governor=schedutil` (the kernel default is
`CONSERVATIVE`, which would itself act as a soft cap).

## Release plumbing (2026-08-06)

`6.12/rc` promoted and wired into the build path: `notify-kernel-build.yml`
now watches `6.12/rc` as well as `6.18/rc` (the workflow file had to be added
on the 6.12 line at all — GitHub reads push-triggered workflows from the pushed
branch, and `git add -f` is required because the kernel `.gitignore` excludes
`.*`); citronics-kernel gained `configs/msm8x74-6.12-rc.config` (nvmem cpufreq,
QCOM_SPM, CPU_THERMAL, step-wise, pstore/ramoops, VADC, schedutil default — all
verified to survive `olddefconfig`) and a `kernels.conf` entry. Its auto-tagger
was fixed (`943c8c2`): it derived the next tag from the *count* of `v3.2-rc3.N`
tags, and the sequence has gaps, so it kept trying to create an existing tag and
aborted before compiling — now max+1 with a collision loop.

The rig DTB is in `arch/arm/boot/dts/qcom/Makefile` so `deb-pkg` ships it;
on-install DTB selection already exists (`zz-citronics-boot`, `02-copy-dtb`),
keyed on `deviceinfo_dtb` with a **phone-DTB fallback** — so a rig install must
write `/etc/deviceinfo` with `deviceinfo_dtb="…-fp2-rig.dtb"` *before*
installing the kernel.

## SD card-detect / `pwr_irq` storm — closed as a retraction, mechanism still open

Recorded here because the tree is right but the reason was nowhere: the revert
(`e850a43667c8`) carries only git's auto-generated message.

- **GPIO 62 is not the FP2's card-detect line.** Vendor authority, in as many
  words: `arch/arm/mach-msm/board-8974-gpiomux.c:754` —
  `//In FP2, gpio 62 wasn't used for sd card detecting. so annotate it.` with
  the whole `sd_card_det` config `#if 0`'d and its `msm_gpiomux_install`
  commented out (line 1478). The pin came from Sony shinano/rhine and from the
  QC reference-board tables in the vendor tree; the FP2 oracle had already said
  the same (gpio62 UNCLAIMED, no CD interrupt, gpio63 = CDC_RESET).
- **Muxing it caused SoC resets** (operator observation, 2026-08-04). Worth
  following up as an XPU lead: TLMM GPIOs have per-pin ownership, and a write
  to a pin owned by another master is exactly the class of access an XPU
  protects — which, with `qcom,xpu-err-fatal` armed, is a PS_HOLD reset. If it
  reproduces, that is a *known-trigger* XPU violation, i.e. a way to read the
  TZ diag region with a violation we caused on purpose.
- **It was also a functional regression, not just a wrong pin.** `mmc_rescan()`
  bails out via `mmc_power_off()` whenever `host->ops->get_cd(host) == 0`, so an
  unconnected CD pin that never reads "present" silences the poll *and* means an
  inserted SD card is never detected.
- **There is no upstream 6.16 fix for this** (checked 2026-08-06):
  `&sdhc_2` in `qcom-msm8974pro-fairphone-fp2.dts` is byte-identical at v6.12,
  v6.16 and v6.18 *and* in the pmaports fork's `v6.16.12-msm8974`; the SoC
  `sdhc_2` node (hc_irq 125 / pwr_irq 221) is unchanged across the same span.
  `db58532188eb` ("Ensure SD card power isn't ON when card removed") landed for
  **6.17**, not 6.16, and is already in our baseline via stable **v6.12.43**
  (`6f38d9ae4b6c`); `20a0c37e4406` (regulator load) is in v6.12.43 too. So the
  earlier "missing 6.16 power-IRQ rework" claim is **retracted** — nothing to
  backport.
- **Mechanism, from the code:** `sdhci_msm_check_power_status()` skips the wait
  when the request already matches `curr_pwr_state`/`curr_io_level`. If the
  power IRQ never arrives, `curr_pwr_state` stays at its probe value
  (`REQ_BUS_OFF`), so BUS_OFF/IO_LOW requests never wait while BUS_ON (2) and
  IO_HIGH (8) always time out — exactly the two warnings observed, once per
  poll, forever.
- **But it is not inherent to this board+driver:** a real FP2 on 6.18.40 with an
  empty, polled slot logs **zero** `pwr_irq` timeouts over a 46 s boot
  (`artifacts/fp2-dut/20260730-preflash-ubuntu/salvage/dmesg.txt`), and
  `MMC_CAP_NEEDS_POLL` is derived identically in 6.12 and 6.18 (sdhci.c, via
  `SDHCI_QUIRK_BROKEN_CARD_DETECTION` + no CD GPIO). So the storm is specific to
  the 6.12 build or the fixture, not to mainline-on-FP2.
- **Next step is one measurement, not another DT guess:** does mmc1's `pwr_irq`
